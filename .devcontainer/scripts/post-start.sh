...
fix_vscode_jsonrpc_node_resolution() {
    local npm_root
    ...
    local squad_root="$npm_root/@bradygaster/squad-cli"
    if [ ! -d "$squad_root" ]; then
        return
    fi

    while IFS= read -r jsonrpc_dir; do
        [ -n "$jsonrpc_dir" ] || continue
        if [ -f "$jsonrpc_dir/node.js" ] && [ ! -e "$jsonrpc_dir/node" ]; then
            ln -s node.js "$jsonrpc_dir/node" 2>/dev/null || true
        fi
    done < <(find "$squad_root" -type d -path "*/vscode-jsonrpc" 2>/dev/null)
}
